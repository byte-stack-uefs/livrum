...
class CouponService:
    ...
    def deletarCupom(self, idCupom: str, idUsuario: str):
        try:
            db.execute(
                "DELETE FROM cupom WHERE idCupom AND idUsuario = %s",
                (idCupom, idUsuario),
            )
            db.commit()
            return True
    ...
    def adicionarCupom(self, novoCoupon: CouponDTO, idUsuario: str) -> Coupon:
        with DB() as db:
            try:
                db.execute(
                    "INSERT INTO cupom (idCupom, nome, idUsuario, porcentagem, dataExpiracao,status,criadoEm,modificadoEM,) VALUES (%s, %s, %s, %s, %s,%s,%s,%s)",
                    (
                        novoCoupon.idCupom,
                        novoCoupon.nome,
                        idUsuario,
                        novoCoupon.porcentagem,
                        novoCoupon.dataExpiracao,
                        novoCoupon.status,
                        novoCoupon.criadoEm,
                        novoCoupon.modificadoEM,
                    ),
                )
                db.commit()
            except:
    ...
    def atualizarCupom(self, idCupom: str, atualizacaoCoupon: CouponDTO) -> Coupon:
        with DB() as db:
            try:
                db.execute(
                    """UPDATE cupom SET nome=%s, porcentagem=%s, dataExpiracao=%s, status=%s, modificadoEM=%s WHERE idCupom=%s""",
                    (
                        atualizacaoCoupon.nome,
                        atualizacaoCoupon.porcentagem,
                        atualizacaoCoupon.dataExpiracao,
                        atualizacaoCoupon.status,
                        atualizacaoCoupon.criadoEm,
                        atualizacaoCoupon.modificadoEM,
                        idCupom,
                    ),
                )
                db.commit()
